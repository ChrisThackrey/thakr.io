...
const contactFormSchema = z.object({
  ...
  turnstileToken: z.string().min(1, { message: "Please complete the CAPTCHA." }),
})
...
export function ContactForm() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 bg-card p-6 sm:p-8 rounded-lg shadow-lg">
      ...
      />
      {errors.turnstileToken && <p className="mt-1 text-sm text-destructive">{errors.turnstileToken.message}</p>}
