      ...
      <div className="container py-12 md:py-16 space-y-12">
        <div className="space-y-6">
          ...
          <AnimateInView animation="slide-up" delay={0.1}>
            <Card className="bg-muted/30 dark:bg-muted/20 border border-border/50 rounded-xl shadow-sm">
              <CardContent className="p-4">
                <div className="flex flex-wrap gap-2 justify-center md:justify-start">
                  {categories.map((category) => (
                    <motion.div key={category} whileHover={{ y: -2 }} whileTap={{ scale: 0.98 }}>
                      <Badge
                        className={`text-sm font-medium py-1.5 px-3 cursor-pointer transition-all rounded-md ${
                          selectedCategory === category
                            ? "bg-primary text-primary-foreground shadow-sm hover:bg-primary/90"
                            : "bg-background/50 text-muted-foreground border border-transparent hover:bg-accent hover:text-accent-foreground hover:border-border"
                        }`}
